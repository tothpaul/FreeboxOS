...
type
  ...
  private
    ...
    procedure BuldJSONNullBoolean(const Value: TNullBoolean);

  public
    procedure Append(const Str: TString); overload;
    procedure Append(Value: Integer); overload;
    procedure AppendString(const Str: UTF8String); overload;
    procedure AppendString(const Str: string); overload;
...
// finally, this Object Helper simplify everything
  ...
  JSON = class
    ...
    class procedure fromJSON<T>(var instance: T; const Str: TString); inline;
...
class procedure JSON.fromJSON<T>(var instance: T; const Str: TString);
var
  Parser: TJSONParser;
begin
  Finalize(instance);
...
begin
  case JSONInstanceType(TypeInfo, Instance) of
    ...
    jsNullBoolean: BuldJSONNullBoolean(TNullBoolean(Instance^));
    ...
    jsWChar   : AppendString(Char(Instance^));
    ...
    jsPointer  : BuildJSON(TypeInfo.TypeData.RefType^, PPointer(Instance)^);
...
begin
  ...
  begin
    if Index > 0 then
      Append(',');
    Append('"');
    Append(TString(Fields[Index].Name));
    Append('":');
...
procedure TJSONBuilder.BuldJSONNullBoolean(const Value: TNullBoolean);
begin
  if Value.IsSet then
...
procedure TJSONBuilder.AppendString(const Str: UTF8String);
begin
...
begin
  ...
  case JSONInstanceType(TypeInfo, Instance) of
    ...
    jsDynArray:
    begin
      Result := DynArraySize(PPointer(Instance)^) > 0;
    end;
    jsDateTime:
    begin
